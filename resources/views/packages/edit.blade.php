<x-app-layout :title="'Edit Packages'">
    ...
    <div class="space-y-6 animate-fade-in-up">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-semibold text-stone-800">Edit Packages</h1>
                <p class="text-sm text-stone-500 mt-0.5">Perbarui informasi Packages <span class="font-medium text-[#E67E22]">{{ $package->name }}</span></p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <textarea id="description" name="description" rows="4"
                                          ...
                                          class="w-full rounded-xl border border-stone-200 bg-stone-50 px-4 py-2.5 text-sm text-stone-800 placeholder-stone-400
                                                 ...
                                                 @error('description') border-red-400 bg-red-50 @enderror">{{ old('description', $package->description) }}</textarea>
                                @error('description')
                                    <p class="flex items-center gap-1.5 text-xs text-red-500">
                                        <i class="ti ti-alert-circle text-sm"></i> {{ $message }}
                                    </p>
                                ...
                                <select id="category_id" name="category_id"
                                    ...
                                    @foreach ($categories as $cat)
                                        <option value="{{ $cat->category_id }}"
                                            @selected(old('category_id', $package->category_id) == $cat->category_id)>
                                            {{ $cat->nama_category }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="space-y-5">
                    ...
                    <div class="rounded-2xl border border-stone-200 bg-white shadow-sm">
                        ...
                        <div class="px-6 py-5" x-data="imagePreview('{{ $package->image_url }}')">
                                ...
                                <div class="relative h-36 w-full overflow-hidden rounded-xl border border-stone-200 bg-stone-50">
                                    ...
                                    <template x-if="previewUrl">
                                        <button type="button" @click="removeImage()"
                                                class="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white shadow hover:bg-red-600 transition">
                            ...
                            <label for="image"
                                ...
                                <span x-text="previewUrl ? 'Ganti Gambar' : 'Pilih Gambar'"></span>
                            </label>
                    ...
                    <div class="rounded-2xl border border-stone-200 bg-white shadow-sm">
                        ...
                        <div class="px-6 py-5">
                            <label class="flex cursor-pointer items-center justify-between gap-3">
                                ...
                                <div x-data="{ on: {{ old('is_active', $package->is_active) ? 'true' : 'false' }} }"
                                     class="relative">
